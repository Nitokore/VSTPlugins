  ...
  float *out4)
{
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (!stickEnvelope.isTerminated) {
      ...
      out0[i] = pulseMix * pulsar.process()
        + stickEnv * (toneMix * stickTone + velvetMix * velvet.process());
    } else {
      out0[i] = 0.0f;
    }
    if (in0 != nullptr) out0[i] += in0[i];

    // FDN.
    if (enableFDN) {
      const float fdnFeedback = interpFDNFeedback.process();
      fdnSig = fdnCascade[0].process(juce::dsp::FastMathApproximations::tanh<float>(
        out0[i] + in1[i] + fdnFeedback * fdnSig));
    ...
    serialAP1Sig = serialAP1.process(
      out1[i] + in2[i] + interpAllpass1Feedback.process() * serialAP1Sig);
    float apOut = serialAP1Highpass.process(serialAP1Sig);
    out2[i] = apOut;

    serialAP2Sig = in3[i] + apOut + interpAllpass2Feedback.process() * serialAP2Sig;
    float sum = 0.0f;
    ...
    out3[i] = out1[i] + allpassMix * (apOut - out1[i]);
    ...
    out4[i] = out3[i]
      + interpTremoloMix.process()
        * ((tremoloDepth * tremoloLFO + 1.0f - tremoloDepth) * tremoloDelay.process(out3[i] + in4[i]) - out3[i]);
