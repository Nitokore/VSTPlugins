...
bool PlugEditor::open(void *parent, const PlatformType &platformType)
{
  ...
  addGroupLabel(leftTremoloRandom, top2, 3.0f * knobX + 2.0f * margin, "Tremolo Random");
  addKnob(
    leftTremoloRandom, topTremolo - 0.5f * margin, smallKnobWidth, colorBlue, " Depth",
    ID::randomTremoloDepth, param.value[ID::randomTremoloDepth]->getDefaultNormalized(),
    LabelPosition::right);
  addKnob(
    leftTremoloRandom + 1.0f * (knobX + margin), topTremolo - 0.5f * margin,
    smallKnobWidth, colorBlue, " Freq", ID::randomTremoloFrequency,
    param.value[ID::randomTremoloFrequency]->getDefaultNormalized(),
    LabelPosition::right);
  addKnob(
    leftTremoloRandom + 2.0f * (knobX + margin), topTremolo - 0.5f * margin,
    smallKnobWidth, colorBlue, " Time", ID::randomTremoloDelayTime,
  ...
  addSplashScreen(
    ...
    viewRect.bottom - 40.0f, "FDNCymbalMod");
